...
const categoryItemsData = [
  {
    ...
    itemPositionSpecific: 'top-[90px] left-[15%]',
    iconContainerSize: 'w-[62px] h-[62px]',
    initialIconScale: 'scale-[1.35]',
    textOffset: 'left-[calc(100%+18px)]'
  },
  {
    name: 'Water',
    count: 55,
    IconComponent: WaterIcon,
    itemPositionSpecific: 'top-[240px] left-[30%]',
  ...
  },
  {
    ...
    itemPositionSpecific: 'top-[390px] left-[10%]',
...
const CategorySidebar: React.FC<CategorySidebarProps> = ({ selectedCategory, onSelectCategory }) => {
  return (
    <div className="relative h-[500px] hidden lg:block">
      {categoryItemsData.map(({ name, count, IconComponent, itemPositionSpecific, iconContainerSize, initialIconScale, textOffset }) => {
        ...
        return (
          ...
          >
            {isSelected && (
              <div
                className="absolute size-[110px] rounded-full border-[10px] border-wonderla-btn-yellow bg-white
            ...
            <div className={`relative ${iconContainerSize} 
                             ...
                             ${isSelected ? 'scale-[1.35]' : initialIconScale}
                           `}>
              <div
                className={`absolute inset-[-8px] rounded-full bg-white shadow-md -z-20 
                            transition-all duration-200 ease-in-out
                            ${isSelected ? 'opacity-60 scale-95' : 'opacity-100 scale-100 group-hover:ring-2 group-hover:ring-wonderla-btn-yellow/50'}`}
              ></div>
              <IconComponent className="w-full h-full text-wonderla-icon-blue overflow-visible" />
            </div>
