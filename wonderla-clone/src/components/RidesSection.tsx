...
const RidesSection = () => {
  ...
  }, [Autoplay({ delay: 2000, stopOnInteraction: true })]); // Autoplay with 5s delay, stops on user interaction

  const handleCategorySelect = (categoryName: string) => {
    setSelectedCategory(categoryName);
    if (emblaApi) emblaApi.scrollTo(0); // Reset carousel to start when category changes
  };
